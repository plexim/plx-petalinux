...
.controller('modelCtrl', ['$scope', '$http', '$httpParamSerializerJQLike', 'xmlrpc', function ($scope, $http, $httpParamSerializerJQLike, xmlrpc) {
   ...
   .then(function(response) {
      ...
      return xmlrpc.callMethod('rtbox.serials',[]);
   })
   .then(function(response) {
      $scope.rtboxType = response.rtboxType;      
   });
}])
...
.controller('tabsCtrl', ['$scope', '$rootScope', '$http', '$interval', '$httpParamSerializerJQLike', '$uibModal',
   ...
   $scope.execTime1 = [ { 'Current': 0, 'Maximum': 0 } ];
   $scope.execTime2 = [ { 'Current': 0, 'Maximum': 0 } ];
   $scope.execTime3 = [ { 'Current': 0, 'Maximum': 0 } ];
   $scope.tempFpga = [ { 'Temperature': 0 } ];
   $scope.fanSpeed = [ { 'Speed': 0 } ];
   $scope.voltages = [[ 0.0, 0.0, 0.0 ]];
   $scope.currents = [[ 0.0, 0.0, 0.0 ]];
   $scope.power = [[ 0.0, 0.0, 0.0 ]];
   $scope.totalPower = [ 0.0 ];
   $scope.appfile = '';
   $scope.sampleTime = 0;
   $scope.execTimeHist1 = [];
   $scope.execTimeHist2 = [];
   ...
   for (var i = 0; i < $scope.histLength; i++) {
       $scope.execTimeHist1.push({ 'Current': 0, 'Maximum': 0 });
       $scope.execTimeHist2.push({ 'Current': 0, 'Maximum': 0 });
       $scope.execTimeHist3.push({ 'Current': 0, 'Maximum': 0 });
   }
   $scope.logPosition = 0;
   ...
   var refreshStatus = function() {
      ...
      .then( function(response) {
         ...
         var i;
         for (i = 0; i< 3; i++) {
            $scope.voltages[0][i] = response.voltages[i].toFixed(1);
            $scope.currents[0][i] = response.currents[i].toFixed(1);
            var power = response.voltages[i] * response.currents[i];
            $scope.power[0][i] = power.toFixed(1);
            totalPower += power;
         }
         $scope.totalPower[0] = totalPower.toFixed(1);
   ...
   var refreshModelInfo = function() {                                  
      ...
      .then( function(response) {                 
         ...
         for (i=0; i<3; i++) {
            if (response.periodTicks[i]) {
               $scope.periodTicks[i] = response.periodTicks[i];
            } else {
               $scope.periodTicks[i] = 1; // avoid division by zero later
            }
         }
         $scope.modelTimeStamp = response.modelTimeStamp;
   ...
   var refreshExecutionTime = function() {
      ...
      .then( function(response) {
         if ($scope.sampleTime) {
            $scope.execTime1[0].Current = response.runningCycleTime1/$scope.periodTicks[0]*100;
            $scope.execTime1[0].Maximum = response.maxCycleTime1/$scope.periodTicks[0]*100;
            $scope.execTime2[0].Current = response.runningCycleTime2/$scope.periodTicks[1]*100;
            $scope.execTime2[0].Maximum = response.maxCycleTime2/$scope.periodTicks[1]*100;
            $scope.execTime3[0].Current = response.runningCycleTime3/$scope.periodTicks[2]*100;
            $scope.execTime3[0].Maximum = response.maxCycleTime3/$scope.periodTicks[2]*100;
            $scope.execTimeHist1.push( {
               'Current': $scope.execTime1[0].Current,
               'Maximum': $scope.execTime1[0].Maximum
            } );
            $scope.execTimeHist2.push( {
               'Current': $scope.execTime2[0].Current,
               'Maximum': $scope.execTime2[0].Maximum
            } );
            $scope.execTimeHist3.push( {
               'Current': $scope.execTime3[0].Current,
               'Maximum': $scope.execTime3[0].Maximum
            } );
         }
         if ($scope.modelTimeStamp != response.modelTimeStamp) {
            refreshModelInfo();
         }
         if ($scope.execTimeHist1.length > $scope.histLength) {
            $scope.execTimeHist1.splice(0, $scope.execTimeHist1.length-$scope.histLength);
         }
         if ($scope.execTimeHist2.length > $scope.histLength) {
            $scope.execTimeHist2.splice(0, $scope.execTimeHist2.length-$scope.histLength);
         }
         if ($scope.execTimeHist3.length > $scope.histLength) {
            $scope.execTimeHist3.splice(0, $scope.execTimeHist3.length-$scope.histLength);
         }
      });
   }
   ...
   $scope.formatExecTime1 = function(value, ratio) {
      if (value == $scope.execTime1[0].Current) {
   ...
   $scope.formatExecTime2 = function(value, ratio) {
      if (value == $scope.execTime2[0].Current) {
         return value.toFixed(0);
      } else {
         return;
      }
   ...
   $scope.formatExecTime3 = function(value, ratio) {
      if (value == $scope.execTime3[0].Current) {
         return value.toFixed(0);
      } else {
         return;
      }
