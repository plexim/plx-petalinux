...
#define IVSHMEM_CFG_STATE_TAB_SZ	0x44
#define IVSHMEM_CFG_RW_SECTION_SZ	0x48
#define IVSHMEM_CFG_OUT_SECTION_SZ	0x50
#define IVSHMEM_CFG_ADDRESS		0x58
					...
					const struct pci_device_id *id)
{
	...
	if (info->mem[1].size == 0) {
		pci_read_config_dword(dev, IVSHMEM_CFG_ADDRESS,
			(u32*)&info->mem[1].addr);
		pci_read_config_dword(dev, IVSHMEM_CFG_ADDRESS + 4,
			((u32*)&info->mem[1].addr) + 1);
		pci_read_config_dword(dev, IVSHMEM_CFG_STATE_TAB_SZ,
			(u32*)&info->mem[1].size);
		info->mem[1].memtype = UIO_MEM_PHYS;
		info->mem[1].name = "state_table";
		pci_read_config_dword(dev, IVSHMEM_CFG_RW_SECTION_SZ,
			(u32*)&info->mem[2].size);
		pci_read_config_dword(dev, IVSHMEM_CFG_RW_SECTION_SZ + 4,
			((u32*)&info->mem[2].size) + 1);
		info->mem[2].addr = info->mem[1].addr + info->mem[1].size;
		info->mem[2].memtype = UIO_MEM_PHYS;
		info->mem[2].name = "shmem";
